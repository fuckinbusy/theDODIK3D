...
/*
 * Рисует текстуру tid из карты mid в прямоугольник (x_start, y_start, w, h).
 * mirrored — отразить по горизонтали.
 */
void render_draw_texture(int x_start, int y_start, int w, int h,
                         TextureMapId mid, TextureId tid, bool mirrored);

/*
 * Рисует строку str шрифтом из FontMap mid.
 * size — размер символа в пикселях на экране.
 * color — цвет заливки (ARGB8888).
 */
