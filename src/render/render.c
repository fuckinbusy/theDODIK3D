                              ...
                              int screen_h, RayResult* ray, u32 tex_id)
{
    ...
    if (!texture) {
        SDL_Log("render: нет текстуры стены id=%u", tex_id);
        return;
    }
...
bool render_init(GameState* gs)
{
    ...
    if (shade_table) {
        ...
        SDL_Log("render: нет памяти для shade_table");
    }

    zbuffer = malloc(gs->game_w * sizeof(float));
    if (!zbuffer) SDL_Log("render: нет памяти для zbuffer");

    SDL_Log("render: инициализирован");
    return true;
}
